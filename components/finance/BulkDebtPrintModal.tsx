...
export const BulkDebtPrintModal: React.FC<BulkDebtPrintModalProps> = ({ isOpen, onClose, students }) => {
    ...
    // For one-by-one image download
    const [isDownloading, setIsDownloading] = useState(false);
    const [downloadQueue, setDownloadQueue] = useState<Student[]>([]);
    ...
    // --- Download Logic (one by one) ---
    const handleStartDownload = () => {
        if (selectedStudents.length > 0) {
            setIsDownloading(true);
            setDownloadQueue(selectedStudents);
        }
    };

    useEffect(() => {
        if (isDownloading && downloadQueue.length > 0) {
            const studentToExport = downloadQueue[0];
            setTimeout(async () => {
    ...
    const studentToRender = useMemo(() => {
        if (isDownloading && downloadQueue.length > 0) return downloadQueue[0];
        if (isCopying && copyQueue.length > 0 && currentCopyIndex < copyQueue.length) return copyQueue[currentCopyIndex];
        return selectedStudents.length > 0 ? selectedStudents[0] : null;
    }, [isDownloading, downloadQueue, isCopying, copyQueue, currentCopyIndex, selectedStudents]);
                    ...
                    <div className="md:col-span-1 space-y-4">
                        ...
                        <div className="space-y-2 pt-4 border-t dark:border-gray-700">
                             <Button onClick={handleStartDownload} isLoading={isDownloading} disabled={selectedStudents.length === 0 || isCopying} className="w-full">
                                 {isDownloading ? `Đang tải ${selectedStudents.length - downloadQueue.length + 1}/${selectedStudents.length}...` : <>{ICONS.download} Tải về các ảnh ({selectedStudents.length})</>}
                             </Button>
